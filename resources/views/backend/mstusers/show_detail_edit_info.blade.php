...
<section class="content">
    <form method="POST" id="search-form" class="form-horizontal" action="{{route('mst_users.post_change_info',['id'=>$user['id']])}}">
      @csrf
      <div class="row">
        <div class="col-md-9">
          ...
          <div class="box box-primary box-solid">
            <div class="box-header with-border">
              <h3 class="box-title">{{trans('labels.backend.users.edit')}}</h3>
                ...
                <div class="row">
                    <div class="col-sm-8">
                        <div class="form-group">
                            ...
                            <div class="col-sm-8">
                                {{$user['code']}}
                                <!-- <input type="text" class="form-control @error('code') is-invalid @enderror" value="{{old('code',$user['code'])}}" name="code"  placeholder="Nhập mã giảng viên"> -->
                            </div>
                        </div>
                    </div>
                    <div class="col-sm-8">
                        <div class="form-group">
                            ...
                            <div class="col-sm-8">
                                {{$user['email']}}
                                <!-- <input type="text" class="form-control @error('email') is-invalid @enderror" value="{{old('email',$user['email'])}}"  name="email" id="inputEmail3" placeholder="Nhập địa chỉ email"> -->
                            </div>
                        </div>
                    </div>
                    <div class="col-sm-8">
                        <div class="form-group">
                            ...
                            <div class="col-sm-8">
                                <input type="text" class="form-control @error('name') is-invalid @enderror" value="{{old('name',$user['name'])}}"  name="name"  placeholder="Nhập họ tên">
                            </div>
                        </div>
                    </div>
                </div>
                            ...
                            <div class="col-sm-8">
                                <select class="form-control @error('gender') is-invalid @enderror" name="gender">
                                    @foreach($dataDefault['MstGender'] as $k => $val)
                                        <option value="{{$k}}" {{(old('gender',$user['gender']) == $k) ? 'selected' : ''}}>{{$val}}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-sm-8">
                                <div class="input-group date">
                                    ...
                                    <input type="text" class="form-control pull-right @error('date_of_birth') is-invalid @enderror" value="{{(old('date_of_birth',date("d/m/Y", strtotime(str_replace('/', '-', $user['date_of_birth'])))))}}" name="date_of_birth" id="datepicker">
                                </div>
                            </div>
                        </div>
                    </div>
                            ...
                            <div class="col-sm-8">
                                <select class="form-control select-mst-title @error('smt_name') is-invalid @enderror" name="id_semester">
                                    @foreach($dataDefault['MstSemester'] as $key => $valueMstSemester)
                                        <option value="{{$key}}" {{(old('id_semester',$user['lg_id_semester']) == $key) ? 'selected' : ''}}>{{$valueMstSemester}}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                            ...
                            <div class="col-sm-8">
                                <select class="form-control select-mst-title @error('mst_title') is-invalid @enderror" name="mst_title">
                                    @foreach($dataDefault['MstTitle'] as $key => $valueTitle)
                                        <option value="{{$key}}" {{(old('mst_title',$user['id_title']) == $key) ? 'selected' : ''}}>{{$valueTitle}}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                            ...
                            <div class="col-sm-8">
                                <select class="form-control select-mst-position @error('mst_position') is-invalid @enderror" name="mst_position">
                                    @foreach($dataDefault['MstPosition'] as $key => $valuePosition)
                                        <option value="{{$key}}"  {{(old('mst_position',$user['id_position']) == $key) ? 'selected' : ''}}>{{$valuePosition}}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                            ...
                            <div class="col-sm-8">
                                <select class="form-control @error('mst_unit') is-invalid @enderror" name="mst_unit">
                                    @foreach($dataDefault['MstUnit'] as $key => $valueUnit)
                                        <option value="{{$key}}" {{(old('mst_unit',$user['id_unit']) == $key) ? 'selected' : ''}}>{{$valueUnit}}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
